...
#### 智能仿写 / 智能 PRD（增强版 v2.6+）

输入需求描述，AI 自动生成完整策划案。v2.6 起采用**五步增强流程**：

**增强流程**：
1. **多分类 RAG 检索** — 自动从知识库检索世界观、系统设定、数值规则、PRD 模板、UI 规范等分类，确保生成内容贴合项目
2. **知识约束检查** — 禁止 AI 自创世界观/职业/玩法/数值，必须基于知识库内容生成，与现有系统冲突时自动提示
3. **模板强制输出** — 参考上传的文档模板，严格保持格式、章节结构、标题层级一致
4. **自检 + 自动重写** — 生成后自动检查完整性、逻辑矛盾、数值合理性，不合格自动重写并记录原因
5. **优化提示词** — 角色定位（资深策划专家）、Few-Shot 示例、输出约束、格式约束、风格约束

**辅助功能**：
- 可选上传脑图/思维导图辅助
- 可选上传系统原型图（PNG，多张），支持 vision 的模型可看图理解需求
- 上传 .docx 模板文件，仿写时严格按模板格式生成

**支持上传的脑图格式**：xmind, mm, png, jpg, txt, md
